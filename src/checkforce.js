...
(function() {
	...
	var CheckForce = function(input,options) {
		 ...
		 * @return {Object}
		 */
		var checkPasswordNode = function(password){
			options.scores = 0;
		...
		*/
		var lengthPassword = function() {
			...
			else if ((pwdlength >= options.minimumChars)
			&& (pwdlength <= options.maximumChars)) {
				options.scores += 10;
		...
		* @return {Integer}
		*/
		var charactersPassword = function() {
			var password = input.value,
			scores = 0,
